  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-500">
      <div className="text-center">
        <h2 className="text-2xl font-bold font-heading text-primary">Design & Inspiration</h2>
        <p className="text-primary/70 mt-2">Pick from our catalog or upload your own references.</p>
      </div>
      ...
      <div className="space-y-10">
        ...
        <div className="border-b border-primary/10 pb-10">
           <h3 className="font-heading text-xl text-primary mb-2">Our Portfolio</h3>
           <p className="text-sm text-primary/60 mb-4">Click any of our past creations to add them to your references.</p>
           ...
           {isLoadingCatalog ? (
                   ...
                   return (
                     <button
                       ...
                       className={`relative w-40 h-40 shrink-0 snap-center rounded-2xl overflow-hidden shadow-sm transition-all duration-300 ${
                          isSelected ? "ring-4 ring-accent scale-95" : "border border-border hover:brightness-110"
                       } ${isDisabled ? "opacity-40 cursor-not-allowed grayscale" : ""}`}
                     >
                        <Image src={imgUrl} alt="Catalog Inspiration" fill className={`object-cover ${isSelected ? "" : "opacity-90"}`} />
                        {isSelected && (
              ...
              <p className="text-sm text-primary/50 italic bg-subtleBackground p-4 rounded-xl text-center">No images available for this category yet.</p>
           )}
        </div>
        ...
        <div className="bg-subtleBackground p-4 rounded-xl border border-primary/10 flex flex-col md:flex-row items-center justify-between gap-4">
           <div>
              <p className="font-bold text-primary">Reference Capacity: <span className={isAtCapacity ? "text-red-500" : "text-accent"}>{currentTotal} / 3</span></p>
              {catalogSelectedImagesCount > 0 && (
                 <p className="text-xs text-primary/60 mt-1">
                   {catalogSelectedImagesCount} catalog image(s) selected. 
                   You can upload up to {maxImages - currentTotal} of your own.
                 </p>
              )}
           </div>
           {isAtCapacity && (
        ...
        {/* ROW 2: UPLOAD ZONE */}
        <div className="border-b border-primary/10 pb-10">
            <h3 className="font-heading text-xl text-primary mb-2">Your Uploads</h3>
            <p className="text-sm text-primary/60 mb-4">Attach files from your device to reference alongside catalog choices.</p>
            
            <MultiImageUpload 
              value={uploadedSelectedImages}
              onChange={(newUploads) => {
                 // Safely merge the new uploads array with whatever catalog images they currently have checked.
                 const currentCatalogPicks = referenceImages.filter(url => catalogImages.includes(url));
                 setValue("referenceImages", [...currentCatalogPicks, ...newUploads], { shouldValidate: true });
              }}
            ...
            {errors.referenceImages && (
               <p className="text-red-500 text-sm mt-4 flex items-center gap-1 font-medium bg-red-50 p-2 rounded-lg">
                 <AlertCircle className="w-4 h-4 shrink-0" /> {errors.referenceImages.message}
               </p>
            )}
        </div>
        ...
        <div className="space-y-6">
             <div className="bg-white p-6 rounded-2xl shadow-sm border border-border">
               <div className="flex items-center space-x-3">
                 <Switch
                   ...
                   onCheckedChange={(checked) => {
                     ...
                     if (!checked) setValue("details.textOnCake", "", { shouldValidate: true });
                   }}
                 />
                 <Label htmlFor="inscription-toggle" className="font-heading font-semibold text-lg cursor-pointer text-primary">
                   Add Cake writing / Inscription <span className="text-primary/50 text-base font-normal tracking-wide">(Optional)</span>
                 </Label>
               </div>
               ...
               <div
                 className={cn(
                   ...
                   showInscriptionInput ? "grid-rows-[1fr] opacity-100 mt-4" : "grid-rows-[0fr] opacity-0 mt-0"
                 )}
               >
                 <div className="min-h-0">
                   <p className="text-xs text-primary/60 mb-3 block">Max 35 characters. Type exactly as you want it written.</p>
                   <Controller
                     control={control}
             ...
             <div className="bg-white p-6 rounded-2xl shadow-sm border border-border">
               <label className="font-heading font-semibold text-lg mb-2 block text-primary">General Design Notes</label>
               <p className="text-xs text-primary/60 mb-3">Colors, themes, specific aesthetic wishes or notes for the baker.</p>
               <Controller
                 control={control}
               ...
               {errors.details?.designNotes && (
                 <p className="text-red-500 text-sm mt-3 flex items-center gap-1 font-medium">
                   <AlertCircle className="w-4 h-4 shrink-0" /> {errors.details.designNotes.message}
                 </p>
               )}
             </div>
        </div>
